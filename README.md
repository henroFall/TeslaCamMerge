...
**NOTE:** FFMPEG just can't do the fast preview encoding with an Intel GPU. I couldn't get a good command line going. So, you will see the full-length clips are processed with hardware, but the fast clips are still processed with software. 

**Update on June 12 (2025):**

This is a merge of many changes that have been made to account for some breaking changes on Tesla's side over the years. There are also some enhancements. 
These include:
* Intel GPU encoder option exposed in TCMConstants
* Days_to_keep override on a folder by folder basis exposed in TCMConstants
* More robust error handling of poorly encoded files, usually Recent Clips
* Proper ignore of thumbnail files to eliminate error in logs (sorry Andre1506, I see what you did there! I might merge your change in later just for fun.)
* Support for event.mp4 files generated by Sentry Mode May 2025+
